...
def main():
    ...
    if nfiles == 1:
        ...
        if re.match(r"([1-9]+)x([1-9]+)$", optlist.layout):
            (rstr, cstr) = re.match(
                r"([1-9]+)x([1-9]+)$", optlist.layout).groups()
    ...
    for prow in range(nprows):
        for pcol in range(npcols):
            if fcnt < nfiles:
                ...
                if optlist.bias:
                    bstring = optlist.bias
                    btype = optlist.btype
                    for hduid in hduids:
                        iu.subtract_bias(bstring, btype, hdulist[hduid])
            ...
            if fcnt == nprows * npcols and fcnt > 1 and not optlist.overlay:
                if fcnt < 6:
                    ax.legend(handles, labels,
                              fontsize='small', title='HDU:Region ')
                else:
                    ax.legend(handles, labels,
                              fontsize='x-small', title='HDU:Region ')
        ...
        for reg in optlist.row or optlist.col:
            ...
            if slice_spec:
                ...
                line_plot(slice_spec, hdulist[hduid].data, optlist.ltype,
                          optlist.steps, map_axis, optlist.offset, name,
                          ax)
            else:
...
def line_plot(slice_spec, hdudata, plot_type, steps,
    ...
    if map_axis == 0:
        ...
        x = np.arange(y1 + 1, y2 + 1)
    ax.plot(x, line1, drawstyle="{}".format(steps),
