...
def parse_args():
    ...
    parser.add_argument("--style", default='default', required=False,
                        choices=style_list)
    parser.add_argument("--drawstyle", default='default', required=False,
                        choices=['steps-mid'])
...
def main():
    ...
    if optlist.title:
        if optlist.title == "auto":
            ...
            if len(filenames) > 1:
                sup_title_string = iu.long_substr(filenames)
        ...
        for reg in optlist.row or optlist.col:
            ...
            if slice_spec:
                ...
                line_plot(slice_spec, hdulist[hduid].data, optlist.ltype,
                          optlist.drawstyle, map_axis, optlist.offset, name,
                          ax)
            else:
...
def line_plot(slice_spec, hdudata, plot_type, drawstyle,
    ...
    ax.plot(x, line1, linestyle=drawstyle,
